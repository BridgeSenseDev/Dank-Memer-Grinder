		...
		<Card.Root>
			<div class="flex flex-row">
				<Card.Header class="w-2/6 pr-0">
					...
					<Card.Description
						>Items will be used again through Dank's DM's after expiry.</Card.Description
					>
				</Card.Header>
				<Card.Content class="flex w-4/6 flex-col justify-center space-y-3 pb-0">
					{#each Object.keys($cfg.autoUse) as autoUseKey (autoUseKey)}
						<div class="flex w-1/2 flex-row items-center space-x-2">
							<Switch id={autoUseKey} bind:checked={$cfg.autoUse[autoUseKey].state} />
